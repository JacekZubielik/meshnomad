...
SP_API void sp_free_port(struct sp_port *port)
{
	...
	if (port->write_buf)
		free(port->write_buf);
#else
	/* Defensive fallback: if a partially-completed sp_open() left the fd
	 * open (a failure path that didn't explicitly close it), close it here
	 * rather than leaking the descriptor and its OS-level exclusive lock
...
#define INIT_OVERLAPPED(ovl) do { \
	...
	if (flock(port->fd, LOCK_EX | LOCK_NB) < 0) {
		/* Close the fd we just opened — sp_free_port() never closes it,
		 * so leaving it open here leaks the descriptor and the OS-level
		 * exclusive lock for the life of the process, permanently EBUSY-ing
		 * every later open() of this port. */
		close(port->fd);
		port->fd = -1;
	...
	if (ioctl(port->fd, TIOCEXCL) < 0 && errno != EINVAL && errno != ENOTTY) {
		/* See fd-leak note above the flock() failure path. */
		close(port->fd);
		port->fd = -1;
