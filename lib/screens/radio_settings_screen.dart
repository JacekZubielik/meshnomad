...
Future<void> pushRadioSettingsScreen(BuildContext context) {
  return Navigator.push(
    context,
    MaterialPageRoute<void>(builder: (context) => const RadioSettingsScreen()),
  );
}
...
/// Applies the duty-cycle limit chosen in the radio settings screen:
...
/// Radio parameter editor — its own screen with a card layout like Region
/// Management (redesign 2026-08-23; used to be an AlertDialog popup opened
/// from the Node settings tile). Fixed-choice fields (preset, bandwidth,
/// spreading factor, coding rate) use the shared -/+ value stepper;
/// selecting a preset still populates every dependent field exactly as the
/// old dropdown did (same _applyPreset path).
...
class _RadioSettingsScreenState extends State<RadioSettingsScreen> {
  ...
  MeshCoreConnector get _connector =>
      Provider.of<MeshCoreConnector>(context, listen: false);
  ...
  void initState() {
    ...
    if (connector.currentFreqHz != null) {
      _frequencyController.text = (connector.currentFreqHz! / 1000.0)
          .toStringAsFixed(3);
    } else {
    ...
    if (connector.currentBwHz != null) {
      ...
      final bwValue = connector.currentBwHz!;
    ...
    if (connector.currentSf != null) {
      ...
      final sfValue = connector.currentSf!;
    ...
    if (connector.currentCr != null) {
      ...
      final crValue = _toUiCodingRate(connector.currentCr!);
    ...
    if (connector.currentTxPower != null) {
      _txPowerController.text = connector.currentTxPower.toString();
    }

    _clientRepeat = connector.clientRepeat ?? false;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      _logRadioSettingsState('Screen initialized');
    });
  }
  ...
  _RadioSettingsSnapshot? _sessionRememberedNonRepeatSnapshot() {
    final snapshot = _connector.rememberedNonRepeatRadioState;
  ...
  void _validateFields() {
    ...
    final maxTxPower = _connector.maxTxPower ?? 22;
  ...
  Future<void> _saveSettings() async {
    ...
    final maxTxPower = connector.maxTxPower ?? 22;
    ...
    final cr = _toDeviceCodingRate(_codingRate.value, connector.currentCr);
    ...
    final knownRepeat = connector.clientRepeat != null;
    ...
    try {
      ...
      await connector.sendFrame(buildSetRadioTxPowerFrame(txPower));
      ...
      if (rememberedSnapshot != null) {
        connector.rememberNonRepeatRadioState(
          rememberedSnapshot.toMeshCoreSnapshot(connector.currentCr),
        );
      }
  ...
  /// Fixed-choice row: label on the left, the shared -/+ stepper on the
  /// right — same control as everywhere else in Settings.
  Widget _stepperRow<T>(
  ...
  Widget build(BuildContext context) {
    ...
    // Preset cycle: when the current values match no preset, a -1 sentinel
    // labeled "Custom" is shown; stepping away from it applies a real
    // preset and the sentinel disappears from the cycle.
    final presetValues = <int>[
      if (_selectedPresetIndex == null) -1,
      ..._visiblePresetIndexes(),
    ];
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      onChanged: (_) =>
                          _handleManualSettingsChanged('frequency'),
                    ...
                    SizedBox(height: t.spacingMd),
                    _stepperRow<LoRaBandwidth>(
                      context,
                      stepperKey: const ValueKey('radioBandwidthStepper'),
                      label: l10n.settings_bandwidth,
                      values: LoRaBandwidth.values,
                      value: _bandwidth,
                      labelOf: (bw) => bw.label,
                      onChanged: (value) {
                        setState(() {
                        ...
                        _logRadioSettingsState(
                          'Manual settings edit: bandwidth',
                        );
                      },
                    ),
                    SizedBox(height: t.spacingMd),
                    _stepperRow<LoRaSpreadingFactor>(
                      context,
                      stepperKey: const ValueKey('radioSpreadingStepper'),
                      label: l10n.settings_spreadingFactor,
                      values: LoRaSpreadingFactor.values,
                    ...
                    SizedBox(height: t.spacingMd),
                    _stepperRow<LoRaCodingRate>(
                      context,
                      stepperKey: const ValueKey('radioCodingRateStepper'),
                      label: l10n.settings_codingRate,
                      values: LoRaCodingRate.values,
                      value: _codingRate,
                      labelOf: (cr) => cr.label,
                      onChanged: (value) {
                        setState(() {
                        ...
                        _logRadioSettingsState(
                          'Manual settings edit: coding rate',
                        );
                      },
                    ),
                    SizedBox(height: t.spacingMd),
                    TextField(
                      ...
                      onChanged: (_) =>
                          _handleManualSettingsChanged('tx power'),
                      decoration: InputDecoration(
                        labelText: l10n.settings_txPower,
                        border: const OutlineInputBorder(),
                        helperText: connector.maxTxPower != null
                            ? '${l10n.settings_txPowerHelper} (max: ${connector.maxTxPower} dBm)'
                    ...
                    Slider(
                      ...
                      onChanged: (value) =>
                          setState(() => _dutyCycle = value.toInt()),
                    ),
                    if (connector.clientRepeat != null) ...[
                      SizedBox(height: t.spacingMd),
                      SwitchListTile(
                    ...
                    SizedBox(
                      ...
                      child: FilledButton(
                        ...
                        onPressed:
                            (_frequencyError != null || _txPowerError != null)
                            ? null
                            : _saveSettings,
